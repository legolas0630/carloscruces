...
interface VaultModuleProps {
  ...
  setVaultDownloads: React.Dispatch<React.SetStateAction<VaultFile[]>>; // Kept for signature type safety
        ...
        {vaultDownloads.length === 0 ? (
          <div className="text-center py-12 border border-dashed border-white/5 bg-white/[0.01] rounded-sm select-none">
            <p className="text-xs text-zinc-500 tracking-wider uppercase font-bold">
              No asset modules deployed to this secure sync node.
            </p>
          </div>
        ) : (
          vaultDownloads.map((file, i) => (
            <div 
              key={i} 
            ...
            >
              <div className="flex-1 min-w-0 select-none">
                {/* Fixed: Input swapped for high contrast read-only typographic text strings */}
                <h3 
              ...
              <div className="flex items-center gap-2 self-start sm:self-auto">
                <button 
                  ...
                  className="text-[0.65rem] font-black tracking-widest bg-zinc-900 border border-white/10 text-zinc-300 hover:bg-[#a8ff00] hover:text-black hover:border-[#a8ff00] px-5 py-2.5 rounded-sm transition-all shadow-md focus:outline-none"
                >
                  DOWNLOAD
                </button>
              </div>
