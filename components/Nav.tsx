...
export default function Nav() {
  ...
  // Dynamic Navigation Engine Schema
  const links = useMemo(() => [
    { name: "MUSIC", href: "/music" },
    { name: "VISUALS", href: "https://xiwame.space", external: true }, // Updated
    { name: "EXPEDITIONS", href: "/expeditions" },
    { name: "MERCH", href: "/merch" },
    { name: "ACCOUNT", href: status === "authenticated" ? "/dashboard" : "/login" },
    { name: "CART", href: "/cart" },
  ], [status]);
  ...
  return (
    <>
      {/* Main Global Navigation Header Grid Row */}
      <motion.nav
        ref={navRef}
   ...
   {/* Desktop Menu Grid Links Container */}
<div className="hidden sm:flex items-center gap-10">
  {links.map((link) => {
    const isActive = pathname === link.href || (link.name === "ACCOUNT" && pathname === "/dashboard");
    const buttonContent = (
      <motion.button
    ...
    return link.external ? (
      ...
      {/* Full-Screen Drawer System Layer overlay */}
      <AnimatePresence>
        {isOpen && (
          <>
            {/* 1. Backdrop Blur Layer Boundary */}
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
            ...
            {/* 2. Content Overlay Portal Canvas */}
            <motion.div
              initial={{ x: "100%" }}
              animate={{ x: 0 }}
              exit={{ x: "100%" }}
            ...
            >
              {/* Dismiss Action Interface Trigger */}
              <button
                onClick={() => setIsOpen(false)}
              ...
              {/* Ambient Glowing Background Node Spot */}
...
{/* Vertical Navigation Links Link Group */}
<div className="flex flex-col items-start gap-4 w-full relative z-10">
  <div className="text-[0.6rem] tracking-[0.4em] text-zinc-700 mb-1 font-bold select-none">INDEX</div>
  {links.map((link, i) => {
    const isActive = pathname === link.href || (link.name === "ACCOUNT" && pathname === "/dashboard");
    const mobileButton = (
      <motion.button
        ...
        animate={{ 
          ...
          x: 0,
          textShadow: isActive 
            ? ["0 0 12px rgba(168,255,0,0.2)", "0 0 25px rgba(168,255,0,0.8)", "0 0 12px rgba(168,255,0,0.2)"] 
    ...
    return link.external ? (
      ...
      <Link key={link.name} href={link.href} className="no-underline w-full">
        {mobileButton}
              ...
              {/* Social Navigation Vector Icons Layout Area */}
              <div className="flex items-center gap-8 border-t border-white/5 pt-6 pb-4 relative z-10 w-full justify-start pl-2 mb-2">
                {socials.map((social, j) => (
                  <motion.a
                    ...
                    className="text-zinc-600 hover:text-[#a8ff00] transition-colors duration-200"
                  >
                    {social.icon}
                  </motion.a>
                ))}
